 ...
 */
@Slf4j
public class TdqTdqMetricProcessWindowFunction extends ProcessWindowFunction<TdqMetric, TdqMetric, String, TimeWindow> {
    public final Map<Long, OutputTag<TdqMetric>> tagMap = new HashMap<>();

    public TdqTdqMetricProcessWindowFunction() {
        for (String tag : new String[]{"10s", "30s", "60s"}) {
            Long seconds = DateUtils.toSeconds(tag);
            tagMap.put(seconds, new OutputTag<>(String.valueOf(seconds), TypeInformation.of(TdqMetric.class)));
        }
    }
    ...
    @Override
    public void process(String tag, Context context, Iterable<TdqMetric> iterable, Collector<TdqMetric> collector) {
