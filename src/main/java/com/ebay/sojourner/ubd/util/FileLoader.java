...
public class FileLoader {
    ...
    public static String loadContent(String absoluteFilePath, String alternativeResource) {
        ...
        try {
            ...
            while ((readBytes = in.read(bytes)) != -1) {
                resultBuilder.append(new String(Arrays.copyOfRange(bytes, 0, readBytes), "utf-8"));
                bytes = new byte[4096];
            }
    ...
    public static InputStream loadInStream(String path, String resource) throws FileNotFoundException {
        ...
        try {
            ...
            e.printStackTrace();
            log.info("Load file failed from path: " + path + ", and try to load from resource: " + resource);
            instream = loadResource(resource);
        }
