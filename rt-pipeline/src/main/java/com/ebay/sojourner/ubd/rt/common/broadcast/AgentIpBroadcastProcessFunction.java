...
@Slf4j
public class AgentIpBroadcastProcessFunction
    extends BroadcastProcessFunction<UbiEvent, AgentIpSignature, UbiEvent> {

  @Override
  public void processElement(UbiEvent ubiEvent, ReadOnlyContext context, Collector<UbiEvent> out)
      throws Exception {
    ReadOnlyBroadcastState<String, Set<Integer>> agentIpBroadcastState =
        context.getBroadcastState(MapStateDesc.agentIpSignatureDesc);
    if (agentIpBroadcastState.contains(ubiEvent.getAgentInfo() + ubiEvent.getClientIP())) {
      ubiEvent
          .getBotFlags()
          .addAll(agentIpBroadcastState.get(ubiEvent.getAgentInfo() + ubiEvent.getClientIP()));
    }
  ...
  public void processBroadcastElement(
      AgentIpSignature agentIpSignature, Context context, Collector<UbiEvent> out)
      throws Exception {
    BroadcastState<String, Set<Integer>> agentIpBroadcastState =
        context.getBroadcastState(MapStateDesc.agentIpSignatureDesc);
    for (Map.Entry<String, Set<Integer>> entry :
        agentIpSignature.getAgentIpBotSignature().entrySet()) {
