...
public class SojournerUBDRTJobForPA {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // SojournerUBDRTJob.class.getResourceAsStream("/ubi.properties");
    ...
    DataStream<GenericRecord> rawEventDataStream =
        executionEnvironment
            .addSource(
                KafkaConnectorFactoryForPA.createKafkaConsumer()
                    .assignTimestampsAndWatermarks(
                        new BoundedOutOfOrdernessTimestampExtractor<GenericRecord>(
                            Time.seconds(10)) {
                          @Override
                          public long extractTimestamp(GenericRecord element) {
                            return ((RheosHeader) element.get("rheosHeader"))
                                .getEventCreateTimestamp();
                          }
                        }))
            .name("Rheos Consumer");
