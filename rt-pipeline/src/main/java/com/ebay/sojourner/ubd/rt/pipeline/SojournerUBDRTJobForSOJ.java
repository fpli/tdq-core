...
//                )).setParallelism(30)
//                .name("Rheos Consumer");
        DataStream<byte[]> binarySourceStream = executionEnvironment.addSource(
                KafkaConnectorFactoryForSOJ.createKafkaConsumer().setStartFromLatest()
        ).setParallelism(30).name("binary source");
...
//        sessionStream.addSink(new DiscardingSink<>()).name("session discarding");
        binarySourceStream.addSink(new DiscardingSink<>()).name("binary source sink").disableChaining();
        // Submit this job
