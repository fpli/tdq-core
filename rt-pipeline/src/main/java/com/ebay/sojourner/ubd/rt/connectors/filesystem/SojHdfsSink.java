...
import static com.ebay.sojourner.ubd.rt.common.constants.PropertiesConstants.*;
 ...
 */
@Slf4j
public class SojHdfsSink<T> extends RichSinkFunction<T> implements CheckpointedFunction, CheckpointListener {
    ...
    private DataFileWriter<T> switchWriter( long eventTime ) throws IOException, InterruptedException {
        ...
        ugi.doAs((PrivilegedExceptionAction<?>) () -> {
            ...
            String eventTaskFileName = indexOfThisSubtask + "-" + chkId + "-" + DateUtil.stampToDate(eventTime, "yyyyMMddHHmm");
            ...
            if (activeStream == null || fileCount.get() % fileRowCount == 0 || !isInInterval) {
                ...
                String fileName = String.format("/.inprogress.part-%s-%s.avro", eventTaskFileName, (UUID.randomUUID()));
                ...
                activeStream = dataFileWriter.create(schema, fileSystem.create(threadLocalActiveFilePath.get(), false, writeBuffer));
    ...
    private void generateFile() throws IOException, InterruptedException {
        ...
        ugi.doAs((PrivilegedExceptionAction<?>) () -> {
            ...
            try {
                fileSystem = FileSystem.get(conf);
                if (threadLocalActiveFilePath.get() != null && fileSystem.exists(threadLocalActiveFilePath.get())) {

                    String s = threadLocalActiveFilePath.get().toString().replace(".inprogress.", "");
                    Path newFilePath = new Path(s.substring(0, s.length() - 42) + "-" + fileCount + "-"
                            + DateUtil.stampToDate(System.currentTimeMillis(), "yyyyMMddHHmmssSSS") + ".avro");

                    if (fileSystem.exists(newFilePath)) {
                        newFilePath = new Path(s + "-" + fileCount + ".avro");
                    }
                    fileSystem.rename(threadLocalActiveFilePath.get(), newFilePath);
                    log.info("persist file: " + threadLocalActiveFilePath.get() + " to " + newFilePath);
                }
            } catch (Exception ex) {
                throw new RuntimeException("error to close previous file handler: " + threadLocalActiveFilePath.get());
            }
            return null;
        });
    }
    ...
    public void initializeState( FunctionInitializationContext context ) throws Exception {
        ...
        ListStateDescriptor<Tuple2<Integer, String>> listStateDescriptor =
                new ListStateDescriptor<Tuple2<Integer, String>>("checkPointTaskFileMapList", TypeInformation.of(new TypeHint<Tuple2<Integer, String>>() {
                }));
        checkPointTaskFileList = context.getOperatorStateStore().getUnionListState(listStateDescriptor);

        //checkPointTaskFileUnionList=context.getOperatorStateStore().getUnionListState(listStateDescriptor);
...
//            }
            ...
            String dateStr = DateUtil.stampToDate(DateUtil.addHourAndGetTime(DateUtil.dateToStamp(failedHHmm, "yyyyMMddHHmm"), -1), "yyyy-MM-dd HH:00:00.000");

            long time = DateUtil.dateToStamp(dateStr, "yyyy-MM-dd HH:mm:ss.SSS");

            String folder2 = DateUtil.stampToDate(time, "'dt='yyyyMMdd'/h='HH");
            String folder1 = DateUtil.stampToDate(DateUtil.dateToStamp(failedHHmm, "yyyyMMddHHmm"), "'dt='yyyyMMdd'/h='HH");

            throw new RuntimeException(String.format("Can't restored ! Failed at %s,suggest to backfill data from %s.\n" +
                            ...
                            "eg: hadoop fs -rmr /sys/edw/working/esa/wrk03_marketing/eip/eip_dcp_dtl_w/avro/%s \n" +
                            "hadoop fs -rmr /sys/edw/working/esa/wrk03_marketing/eip/eip_dcp_dtl_w/avro/%s  etc\n"
...
//
//        log.info("-------xixliu:onProcessingTime currentProcessingTime:" + processingTimeService.getCurrentProcessingTime());
//
//    }
    ...
    public static void main(String[] args ){
        ...
        System.out.println(SojHdfsSink.class.getClassLoader().getResource("com/ebay/sojourner/ubd/rt/connectors/filesystem/KeytabHdfsFactory.class").getPath());
        System.out.println(SojHdfsSink.class.getResource("/").getPath());
//        Files.copy()
