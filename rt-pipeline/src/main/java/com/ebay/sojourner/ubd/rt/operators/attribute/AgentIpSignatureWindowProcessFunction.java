...
public class AgentIpSignatureWindowProcessFunction
        extends ProcessWindowFunction<AgentIpAttributeAccumulator, AttributeSignature, Tuple, TimeWindow> {
    ...
    private AttributeSignature agentIpSignature;
    ...
    public void process( Tuple tuple, Context context, Iterable<AgentIpAttributeAccumulator> elements,
        ...
        if (botFlagList != null && botFlagList.size() > 0) {
            ...
            agentIpSignature.getAttributeSignature().put("agentIp" + agentIpAttribute.getAgent() + agentIpAttribute.getClientIp(), botFlagList);
...
//        getRuntimeContext().getExecutionConfig().getGlobalJobParameters().toMap();
        ...
        agentIpSignature = new AttributeSignature();
    }
