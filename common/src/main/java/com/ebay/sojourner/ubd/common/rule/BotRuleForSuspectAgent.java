...
public class BotRuleForSuspectAgent implements Rule<AgentAttribute> {
    ...
    public int getBotFlag(AgentAttribute agentAttribute) {
        ...
        if (totalSessionCnt > SESSION_CNT_THRESHOLD) {
            ...
            if ((agentAttribute.getNocguidSessionCnt() * 1.0f) > nocguidBaseline || (agentAttribute.getSpsSessionCnt() * 1.0f) > spsBaseline
                    || (agentAttribute.getNouidSessionCnt() * 1.0f) > directVisitBaseline || (agentAttribute.getDirectSessionCnt() * 1.0f) > directVisitBaseline
                    || (agentAttribute.getMktgSessionCnt() * 1.0f) > directVisitBaseline || (agentAttribute.getIpCountForSuspect() * 1.0f) < (totalSessionCnt * 0.01f)) {
                return 220;
            }
        }
        return 0;
