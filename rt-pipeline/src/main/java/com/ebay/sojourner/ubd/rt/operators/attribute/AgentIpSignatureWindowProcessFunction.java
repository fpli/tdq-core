...
public class AgentIpSignatureWindowProcessFunction
        extends ProcessWindowFunction<AgentIpAttributeAccumulator, Tuple2<String, Set<Integer>>, Tuple, TimeWindow> {
    ...
    public void process(Tuple tuple, Context context, Iterable<AgentIpAttributeAccumulator> elements,
        ...
        if (context.currentWatermark() > context.window().maxTimestamp()) {
            out.collect(new Tuple2<>("agentIp" + agentIpAttribute.getAgent() + agentIpAttribute.getClientIp(), null));
            ...
            out.collect(new Tuple2<>("agentIp" + agentIpAttribute.getAgent() + agentIpAttribute.getClientIp(), agentIpAttribute.getBotFlagList()));
        }
    }
