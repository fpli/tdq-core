...
@Slf4j
public class AgentBroadcastProcessFunction
  ...
  public void processElement(
    ...
    ReadOnlyBroadcastState<String, Set<Integer>> attributeSignature =
        context.getBroadcastState(MapStateDesc.agentSignatureDesc);
    if (attributeSignature.contains("agent" + ubiSession.getUserAgent())) {
      ubiSession
          .getBotFlagList()
          .addAll(attributeSignature.get("agent" + ubiSession.getUserAgent()));
    }
  ...
  public void processBroadcastElement(
      Tuple2<String, Set<Integer>> attributeSignature, Context context, Collector<UbiSession> out)
      throws Exception {
    BroadcastState<String, Set<Integer>> agentBroadcastState =
        context.getBroadcastState(MapStateDesc.agentSignatureDesc);
