    ...
    public static CalendarInterval parseDateInterval(String s) {
        ...
        if (!m.matches() || s.equals("interval")) {
            ...
            if (m.group(5) != null || m.group(6) != null || m.group(7) != null ||
                    m.group(8) != null || m.group(9) != null) {
                throw new IllegalArgumentException("Interval specified should not " +
                        "contain any of hour/minute/second/millisecond/microsecond");
            }
            return new CalendarInterval((int) months, microseconds);
        }
    }
