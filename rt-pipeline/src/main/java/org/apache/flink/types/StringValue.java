 ...
 * <p>
 * The mutability allows to reuse the object inside the user code, also across invocations. Reusing
 * a StringValue object helps to increase the performance, as string objects are rather heavy-weight
 * objects and incur a lot of garbage collection overhead, if created and destroyed en masse.
 *
 ...
 */
@Public
public class StringValue implements NormalizableKey<StringValue>, CharSequence,
  ...
  private static final ThreadLocal<char[]> charBuffer = ThreadLocal
      .withInitial(() -> new char[SHORT_STRING_MAX_LENGTH]);

  static {
    System.out.println("our new defined String Value");
  }
  ...
  /**
   * Sets the value of this <code>StringValue</code>, assuming that the binary data is ASCII coded.
   * The n-th character of the
   * <code>StringValue</code> corresponds directly to the n-th byte in the given array after the
   * offset.
   *
   ...
   * @return The substring.
   * @throws IndexOutOfBoundsException Thrown, if the start is negative, or the end is larger than
   * the length.
   */
  ...
  /**
   * Finds any occurrence of the <code>str</code> character sequence in this StringValue. The search
   * starts at position <code>start</code>.
   *
   * @return The position of the first occurrence of the search string in the string value, or
   * <code>-1</code>, if the character sequence was not found.
   */
