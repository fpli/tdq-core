...
public class BotAgentFilter implements Filter<String> {
    ...
    public BotAgentFilter(Collection<String> browserAgents, Collection<String> botAgents) {
        initAgents(browserAgents, browserAgentStarts, browserAgentEnds, browserAgentContains, browserAgentMultiContains);
    ...
    public boolean filter(String agent) throws IOException {
        ...
        if (match(agent, botAgentStarts, botAgentEnds, botAgentContains, botAgentMultiContains)) {
            return true;
        } else if (agent.contains(ADOBE_AIR) &&
                !match(agent, browserAgentStarts, browserAgentEnds, browserAgentContains, browserAgentMultiContains) &&
                !matchMulti(agent)) {
            return true;
        }
     ...
     * @return
     */
    private boolean match(String agent, List<String> agentStarts, List<String> agentEnds, List<String> agentContains, List<List<String>> agentMultiContains) {
    ...
    private void initAgents(Collection<String> agents, List<String> agentStarts, List<String> agentEnds, List<String> agentContains, List<List<String>> agentMultiContains) {
    ...
    private boolean matchMulti(String agent) {
        if (agent.contains("Sleipnir") &&
                (agent.contains("Mobile") || agent.contains("iPhone") || agent.contains("iPad") || agent.contains("iPod"))) {
            return true;
        } else if (agent.contains("Chrome/") && !agent.contains("chromeframe")) {
            return true;
        } else if (agent.startsWith("Dalvik/") && !agent.contains("Safari")) {
            return true;
        }
        return false;
    }
