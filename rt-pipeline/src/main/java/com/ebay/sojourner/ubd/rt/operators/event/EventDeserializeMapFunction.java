...
public class EventDeserializeMapFunction extends RichMapFunction<byte[], UbiEvent> {
  ...
  public UbiEvent map(byte[] rawEvent) throws Exception {
    RheosEvent rheosEvent =
        RheosEventSerdeFactory.getRheosEventHeaderDeserializer().deserialize(null, rawEvent);
    ...
    if (!(genericRecord.get("clientData") instanceof Map)) {
      logger.info("clientData is not of type Map. " + genericRecord.get("clientData"));
      return null;
    }
    ...
    ubiEvent.setSojDataDt(
        SOJTS2Date.castSojTimestampToDate(getLong(getString(genericRecord.get("eventTimestamp")))));
