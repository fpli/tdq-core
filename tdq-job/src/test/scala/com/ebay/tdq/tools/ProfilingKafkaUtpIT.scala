 ...
 * @author juntzhang
 */
case class ProfilingKafkaUtpIT(profiler: String) extends ProfilingJob {
  ...
  override def setup(args: Array[String]): Unit = {
    setupDB(
      """
        ...
        |        "topics": "marketing.tracking.staging.events.total",
  ...
  def test(): Unit = {
    submit(Array[String](
      "--flink.app.name", "tdq.dev.utp", "--flink.app.local", "true"
    ))
  }
}
