...
public class SqlEventRule implements Rule<UbiEvent> {
    ...
    private void setupContext() {
        ...
        rootSchema.add("square",
            ScalarFunctionImpl.create(UdfManager.SquareFunction.class, "eval"));

        // Create planner
        CalciteCatalogReader reader = new CalciteCatalogReader(CalciteSchema.from(rootSchema),
            ImmutableList.of(),
            new JavaTypeFactoryImpl(), new CalciteConnectionConfigImpl(new Properties()));

        SqlOperatorTable opTable =
            ChainedSqlOperatorTable.of(SqlStdOperatorTable.instance(), reader);
    ...
    public int getBotFlag(UbiEvent ubiEvent) {
        ...
        try {
            ...
            if (interpreter.any()) {
                botFlag = (int) interpreter.<Object[]>first()[0];
            }
            long end = System.nanoTime();
